...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        if (nil != self.detailsLabelText) {
            ...
            CGRect labelRect = [detailsLabel.text boundingRectWithSize:CGSizeMake(frame.size.width - 4*margin, maxHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:detailsLabel.font} context:nil];
            CGSize labelSize = labelRect.size;
//            = [detailsLabel.text sizeWithFont:detailsLabel.font constrainedToSize:CGSizeMake(frame.size.width - 4*margin, maxHeight) lineBreakMode:detailsLabel.lineBreakMode];
